...
export class TimeScale<HorzScaleItem> implements ITimeScale {
	...
	public timeToIndex(time: InternalHorzScaleItem, findNearest: boolean): TimePointIndex | null {
		...
		if (this._horzScaleBehavior.key(time) > this._horzScaleBehavior.key(this._points[this._points.length - 1].time)) {
			// special case
			i = findNearest ? this._points.length - 1 as TimePointIndex : null;
		}

		if (i === null) {
			const lastData = this._points[this._points.length - 1]
			
			return (lastData as any).pointData._internal_index + Math.floor(((time as any)._internal_timestamp - (lastData.originalTime as number)))
		}
